...
\paragraph{Challenge 1: Device capture.} How do we determine the area a device is in? Application level registration and polling seems really naive.  We need to use IP for data transfer, but can we use lower levels of the network stack to determine when a device enters/leaves an area? or to determine specifically where a device is? How do other mobile devices do this?

\paragraph{Challenge 2: Scale.} How to handle internet-of-things scenarios? How can we store many, many nodes in a highly dynamic environment effectively so that lookups and record changes can happen in real-time? Because this certainly has a soft real-time requirement.  How can we transmit and accept messages over low bandwidth and high latency networks effectively?

\paragraph{Challenge 3: Dynamic usage management.} How to we provide usage management in this domain? we should have a lot to say here as we have significant work in this area.
