...
As stated in the topic description, the requirement is to ``enable on-the-move warfighters, equipped with state-of-the-art wireless information appliances (e.g., handheld, commercially-available or militarized, or wearable computers), to securely send and receive information that can be targeted and tailored for sender-specified geographic regions, sectors, or operating areas.

\sbircallout{The technical challenges.}{Summarize challenges here.}

\sbirinline{Challenge 1: Device capture.} How do we determine the area a device is in? Application level registration and polling seems really naive.  We need to use IP for data transfer, but can we use lower levels of the network stack to determine when a device enters/leaves an area? or to determine specifically where a device is? How do other mobile devices do this?

\sbirinline{Challenge 2: Scale.} How to handle internet-of-things scenarios? How can we store many, many nodes in a highly dynamic environment effectively so that lookups and record changes can happen in real-time? Because this certainly has a soft real-time requirement.  How can we transmit and accept messages over low bandwidth and high latency networks effectively?

\sbirinline{Challenge 3: Dynamic usage management.} How to we provide usage management in this domain? we should have a lot to say here as we have significant work in this area.
...
\sbirsection{Phase I Statement of Work}
...
Due to space limitations in this proposal, we focus our discussion most heavily on Tasks 2 and 4 in defining the ASW F2F methodology and architecture summarizing the other tasks at a higher level, since these other tasks are derived from Tasks 2 and 4 and are therefore somewhat self-explanatory.
...
The system has a wide requirements domain, stretching from dynamic information usage management to data storage to message delivery schemes.  In this phase, we will identify and enumerate system requirements, aiming for 90\% requirements coverage overall and 100\% requirements coverage with respect to core phase I functionality.  We will name and describe these requirements in a form suitable for future system development or documentation.  This analysis includes identification of sponsor POC geospatial technology used in future tasking.

The primary deliverable from this task will be a document describing these requirements such that the requirements can be traced both forward into the actual system and backward to the initial requirements source.
...
\heading{Task 4a: System Development}
...
\heading{Task 6: Findings}
Finally, at the end of phase I, we will collect and submit our findings to the program sponsor along will all other developed system documentation, source code, and other applicable artifacts.
